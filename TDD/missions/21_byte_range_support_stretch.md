...
Optionally parse `#EXT-X-BYTERANGE` metadata for media segments.

## Red: test prompt

Given byte-range tag before `media.ts`, expect range attached to segment.

## Rust focus

struct modeling, Option, numeric parsing.
...
Stretch only; do not block core downloader.
...
See `../HINTS_AND_SOLUTIONS.md` and search for `Mission 21` for hints and solution shape.
