...
Join a segment beside the playlist URL.

## Red: test prompt

`https://x/a/master.m3u8` + `seg.ts` -> `https://x/a/seg.ts`.

## Rust focus

url crate, Result mapping.
...
Prefer `Url::join` over manual slicing.
...
See `../HINTS_AND_SOLUTIONS.md` and search for `Mission 23` for hints and solution shape.
