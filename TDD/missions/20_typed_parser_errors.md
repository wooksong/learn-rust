...
Introduce meaningful library errors for parser failures.

## Red: test prompt

Invalid bandwidth should return `InvalidBandwidth`; empty variants should return `NoVariants`.

## Rust focus

thiserror, Result, error enums.
...
CLI can still wrap with anyhow later.
...
See `../HINTS_AND_SOLUTIONS.md` and search for `Mission 20` for hints and solution shape.
