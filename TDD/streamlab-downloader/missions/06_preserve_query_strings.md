...
Keep query parameters in segment URLs unchanged.

## Red: test prompt

Given `seg.ts?m=123`, expect exactly `seg.ts?m=123`.

## Rust focus

string preservation, no premature parsing.
...
Do not split on `?` in the media parser.
...
See `../HINTS_AND_SOLUTIONS.md` and search for `Mission 06` for hints and solution shape.
