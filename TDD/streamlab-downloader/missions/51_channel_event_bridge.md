...
Send downloader events to the TUI over an async channel.

## Red: test prompt

Fake downloader sends events; receiver updates state in order.

## Rust focus

tokio::sync::mpsc, async loops.
...
TUI owns state; downloader sends events.
...
See `../HINTS_AND_SOLUTIONS.md` and search for `Mission 51` for hints and solution shape.
