...
Create render-ready state for a display-only TUI.

## Red: test prompt

Default state has idle status and zero progress.

## Rust focus

struct defaults, state modeling.
...
Avoid storing downloader internals.
...
See `../HINTS_AND_SOLUTIONS.md` and search for `Mission 47` for hints and solution shape.
