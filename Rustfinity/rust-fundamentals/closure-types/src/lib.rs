pub fn create_typed_closures() -> (
    ...
    let calculate_total = |total: f32, tax_rate: f32| total + total * tax_rate;
    let apply_discount = |total: &mut f32, discounts: f32| *total -= discounts;
    let checkout_cart = move |items: String| format!("Checkout complete: {}", items).to_string();
