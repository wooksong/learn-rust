# Rust Fundamentals Track

Master the basics of Rust programming language, from syntax to ownership concepts.

## Progress Dashboard

### Beginner Level

- [x] Printing Hello World `[BEGINNER]`
- [x] Declaring Variables `[BEGINNER]`
- [x] Mutable Variables `[BEGINNER]`
- [x] Constants `[BEGINNER]`
- [x] Primitive Data Types `[BEGINNER]`
- [x] Converting Numerical Types `[BEGINNER]`
- [x] Mathematical operations `[BEGINNER]`
- [x] Sum of Array `[BEGINNER]`
- [x] Tuples `[BEGINNER]`
- [x] The Unit Type `[BEGINNER]`
- [x] Functions `[BEGINNER]`
- [x] Control Flow `[BEGINNER]`
- [x] Ownership `[EASY]`
- [x] Mutable References `[BEGINNER]`
- [x] Ownership Rules `[EASY]`
- [x] Character counting string `[BEGINNER]`
- [x] Slices `[EASY]`
- [x] Mutable Slices `[EASY]`
- [x] Slice Manipulation `[MEDIUM]`
- [x] Structs `[EASY]`
- [x] Tuple Structs `[EASY]`
- [x] Constructors `[EASY]`
- [x] Unit Structs `[EASY]`
- [x] Methods on Structs `[EASY]`
- [x] Implementing Debug Trait for Structs `[EASY]`
- [x] Enums `[EASY]`
- [x] Playing Cards `[EASY]`
- [ ] Complex Enums `[MEDIUM]`
- [ ] Methods on Enums `[MEDIUM]`
- [ ] Deriving PartialEq `[EASY]`
- [x] If Let Enums `[EASY]`
- [x] Vectors `[EASY]`
- [x] Hashmaps `[EASY]`
- [ ] Animal sanctuary registry `[MEDIUM]`
- [ ] Student Grades Tracker `[MEDIUM]`
- [ ] Student Grades Tracker 2 `[MEDIUM]`
- [x] Using Option<T> for Optional Values `[EASY]`
- [x] Graceful error handling `[EASY]`
- [x] Graceful error handling 2 `[MEDIUM]`
- [ ] Error Propagation in File Handling `[EASY]`
- [ ] Option Propagation in Rust `[EASY]`
- [ ] Result to Option `[EASY]`
- [ ] Option to Result `[EASY]`
- [x] Panic `[MEDIUM]`
- [ ] unwrap and expect `[MEDIUM]`
- [x] Traits `[EASY]`
- [x] Generics `[EASY]`
- [x] Trait Bounds `[EASY]`
- [ ] The AsRef Trait `[MEDIUM]`
- [ ] The From trait `[MEDIUM]`
- [ ] Returning impl Trait `[EASY]`
- [ ] Return Trait Objects `[MEDIUM]`
- [ ] Trait Objects `[MEDIUM]`
- [ ] Associated types `[MEDIUM]`
- [ ] Supertraits `[MEDIUM]`
- [ ] Plugin System `[HARD]`
- [ ] Lifetimes `[MEDIUM]`
- [ ] Structs and references `[MEDIUM]`
- [ ] Mutable Struct References `[MEDIUM]`
- [ ] Closures `[BEGINNER]`
- [ ] Closure Types `[MEDIUM]`
- [ ] Iterators `[MEDIUM]`
- [ ] Unique Items `[MEDIUM]`
- [ ] Box<T> `[EASY]`
- [ ] The Drop Trait `[MEDIUM]`
- [ ] Rc<T> `[MEDIUM]`
- [ ] Interior Mutability `[MEDIUM]`
- [ ] Concurrency `[MEDIUM]`
- [ ] Channels `[MEDIUM]`
- [ ] Shared State Concurrency `[MEDIUM]`
- [ ] Advanced Match Patterns `[HARD]`
- [ ] Advanced Traits `[MEDIUM]`
- [ ] Macros `[EASY]`
- [ ] Macros 2 `[MEDIUM]`
- [ ] The Default Trait `[EASY]`
- [ ] Procedural macros `[HARD]`

## Statistics

- **Total Problems**: 76
- **Completed**: 37
- **Remaining**: 39
- **Progress**: 48.7%

---

*Track source: [Rustfinity - Rust Fundamentals](https://www.rustfinity.com/tracks/rust-fundamentals)*

